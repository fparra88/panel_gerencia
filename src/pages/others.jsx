...
function PageReportes() {
  ...
  rp_uE(() => { (async () => {
    const t = new Date();
    const f1 = new Date(t.getFullYear(), t.getMonth(), 1).toISOString().slice(0,10);
    const f2 = t.toISOString().slice(0,10);
    setVentas(await window.api.ventasMes(f1, f2));
  })(); }, []);

  const byUser = {};
  ventas.forEach(v => { byUser[v.usuario] = (byUser[v.usuario] || 0) + (v.cantidad * v.precio); });
  const usuarios = Object.entries(byUser).map(([label, value]) => ({ label, value, color: 'var(--c1)' }));

  return (
    <div className="page">
      <div className="section-header">
        <div><h2 className="section-title">Reportes de ventas</h2><p className="section-subtitle">Exporta y analiza ventas por usuario, producto, fecha.</p></div>
        <button className="btn btn-secondary btn-sm"><Icon name="download" size={13}/> Descargar CSV</button>
      </div>
      <div className="dash-grid">
        <div className="card">
          <div className="card-header"><h3 className="card-title">Ventas por vendedor</h3></div>
          <div className="card-body"><window.Charts.HBarChart data={usuarios}/></div>
        </div>
        <div className="card">
          <div className="card-header"><h3 className="card-title">Resumen</h3></div>
          <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div><div style={{ fontSize: 11, color: 'var(--fg-2)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Total del mes</div><div style={{ fontSize: 28, fontWeight: 600, marginTop: 4 }}>{window.fmt.mxn(ventas.reduce((s,v) => s + v.cantidad * v.precio, 0))}</div></div>
            <div><div style={{ fontSize: 11, color: 'var(--fg-2)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Transacciones</div><div style={{ fontSize: 20, fontWeight: 600, marginTop: 4 }}>{ventas.length}</div></div>
            <div><div style={{ fontSize: 11, color: 'var(--fg-2)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Plataformas</div><div style={{ fontSize: 20, fontWeight: 600, marginTop: 4 }}>{new Set(ventas.map(v => v.plataforma)).size}</div></div>
          </div>
        </div>
      </div>
      <div className="card" style={{ marginTop: 16 }}>
        <div className="card-header"><h3 className="card-title">Detalle de ventas</h3></div>
        <div className="table-wrap">
          <table className="table">
            <thead><tr><th>ID</th><th>Fecha</th><th>Producto</th><th>Cliente</th><th>Plataforma</th><th>Pago</th><th className="td-right">Cant.</th><th className="td-right">Total</th></tr></thead>
            <tbody>
...
function PageFull() {
  ...
  rp_uE(() => { (async () => {
    ...
    if (typeof window.api.traspasos === 'function') {
      setTraspasos(await window.api.traspasos());
    }
  })(); }, []);
      ...
      <div className="dash-grid">
        <div className="card">
          ...
          <div className="card-body" style={{ display: 'grid', gap: 12 }}>
            <div className="field"><label className="field-label">Destino</label>
              <select className="select"><option>Cleanest Choice</option><option>Mercado Libre FULL</option></select></div>
            <div className="field"><label className="field-label">SKU</label>
              <select className="select">
                {productos.length === 0
                  ? <option>— Sin productos disponibles —</option>
                  : productos.map(p => <option key={p.sku}>{p.sku} — {p.nombre}</option>)}
              </select></div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div className="field"><label className="field-label">Cantidad</label><input className="input" type="number" defaultValue={50}/></div>
              <div className="field"><label className="field-label">Fecha envío</label><input className="input" type="date" defaultValue={new Date().toISOString().slice(0,10)}/></div>
            </div>
            <button className="btn btn-primary btn-sm" style={{ marginTop: 4 }} onClick={() => toast.success('Traspaso registrado', 'Se notificó a logística')}><Icon name="send" size={13}/> Registrar traspaso</button>
          </div>
        </div>
        <div className="card">
            ...
            {traspasos.length === 0 ? (
              ...
              <div key={t.id || i} className="activity-item" style={{ padding: '12px 20px' }}>
                ...
                <div className="activity-body">
                  <div className="activity-title">{t.dest || t.destino} · <span className="mono">{t.sku}</span></div>
                  <div className="activity-meta">{window.fmt.date(t.date || t.fecha)} · {t.estado}</div>
                </div>
